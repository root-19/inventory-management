...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['sell_product'])) {
    ...
    if ($product_details['quantity'] >= $quantity_sold) {
        ...
        $total_price = $quantity_sold * $product_details['price'];
        ...
        echo '<script>alert("Insufficient quantity available for sale!");</script>';
    }
}
?>
    ...
    <div class="flex">
        ...
        <div class="bg-white shadow-md rounded-lg p-4">
            ...
            <form method="post">
                ...
                <button type="button" onclick="confirmSale(<?php echo $product['id']; ?>, '<?php echo $product['name']; ?>')" class="bg-blue-500 text-white px-4 py-2 rounded-lg mt-4 hover:bg-blue-600">Sell</button>
            </form>
        </div>
    <?php endforeach; ?>
            </div>
     ...
     <script src="https://cdn.jsdelivr.net/npm/sweetalert2@10"></script>
    <script>
        function confirmSale(productId, productName) {
            Swal.fire({
                title: 'Confirm Sale',
                text: `Are you sure you want to sell ${productName}?`,
                icon: 'warning',
